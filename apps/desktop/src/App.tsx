...
export default function App() {
  ...
  const content = {
    home: <Home snapshot={snapshot} busy={busy} onEngine={(enabled) => void act(() => api.setEngine(enabled))} onVolume={(masterVolume) => { const next = { ...draft, masterVolume }; setDraft(next); void commitEffects(next); }} onNavigate={setPage} onTest={() => void api.testSound().catch((reason: unknown) => setError(String(reason)))} />,
    ...
    packs: <Packs snapshot={snapshot} onActivate={(id) => void act(() => api.activatePack(id))} onPreview={(id) => void api.previewPack(id).catch((reason: unknown) => setError(String(reason)))} onImportFolder={() => void importPack(true)} onImportManifest={() => void importPack(false)} />,
    settings: <Settings snapshot={snapshot} onPreferences={(release, repeats, startup, keepRunning) => void act(() => api.updatePreferences(release, repeats, startup, keepRunning))} onOutput={(name) => void act(() => api.selectOutputDevice(name))} onKeyboardAccess={() => void act(() => api.openKeyboardSettings()).catch(() => undefined)} onRestart={() => void api.restart().catch((reason: unknown) => setError(String(reason)))} />,
  }[page];
